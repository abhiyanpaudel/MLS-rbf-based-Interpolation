...
KOKKOS_INLINE_FUNCTION
void VandermondeMatrix(ScratchMatView V, ScratchMatView local_source_points, int j){
...
// moment matrix 
KOKKOS_INLINE_FUNCTION
void PTphiMatrix(ScratchMatView pt_phi, ScratchMatView V, ScratchVecView Phi, int j){
   int M = V.extent(0);
   int N = V.extent(1);
...
KOKKOS_INLINE_FUNCTION
void PhiVector(ScratchVecView Phi, Coord target_point, ScratchMatView local_source_points, int j, double cuttoff_dis_sq){
    int N = local_source_points.extent(0);
...
KOKKOS_INLINE_FUNCTION
void MatMatMul(member_type team, ScratchMatView moment_matrix, ScratchMatView pt_phi, ScratchMatView vandermonde){
...
KOKKOS_INLINE_FUNCTION
void MatVecMul(member_type team, ScratchVecView vector, ScratchMatView matrix, ScratchVecView result){
...
// moment matrix 
KOKKOS_INLINE_FUNCTION
void PtphiPMatrix(ScratchMatView moment_matrix, member_type team, ScratchMatView pt_phi, ScratchMatView vandermonde){
    int M = pt_phi.extent(0);
    int N = vandermonde.extent(1);
    int K = pt_phi.extent(1); 
...
// inverse matrix
KOKKOS_INLINE_FUNCTION
void inverse_matrix(member_type team, ScratchMatView matrix, ScratchMatView lower, ScratchMatView forward_matrix, ScratchMatView solution) {
