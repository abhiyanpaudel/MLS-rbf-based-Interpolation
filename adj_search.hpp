...
class FindSupports {
  ...
  FindSupports(Mesh& source_mesh_, Mesh& target_mesh_): source_mesh(source_mesh_), target_mesh(target_mesh_){};
...
void  FindSupports::adjBasedSearch(const Real& cutoffDistance, const Write<LO>& supports_ptr, Write<LO>& nSupports, Write<LO>& support_idx){   
  ...
  // CSR data structure of adjacent vertex information of each source vertex
  // dim == 2; ask vertices of tri (3 vertices for each tri) & if dim ==3; ask vertices of tetrahedron (4 vertices for each tet) 
    const auto& cells2verts = source_mesh.ask_verts_of(dim);
    
    Kokkos::View<pcms::Real*[2]> target_points("test_points", nvertices_target); 

    parallel_for(nvertices_target, OMEGA_H_LAMBDA(const LO i){
    ...
    parallel_for(nvertices_target, OMEGA_H_LAMBDA(const LO id){
