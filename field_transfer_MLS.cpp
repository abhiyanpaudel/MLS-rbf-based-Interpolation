...
int main(int argc, char** argv) {
  ...
  auto target_mesh =
      build_box(world, OMEGA_H_SIMPLEX, 1, 1, 1, 40, 40, 0, false);
  auto source_mesh =
      build_box(world, OMEGA_H_SIMPLEX, 1, 1, 1, 10, 10, 0, false);
  if (argc != 2) {
  ...
  if (!rank) {
    fprintf(stderr, "target mesh <v e f r> %d %d %d %d\n",
            target_mesh.nglobal_ents(0), target_mesh.nglobal_ents(1),
            target_mesh.nglobal_ents(2),
            dim == 3 ? target_mesh.nglobal_ents(3) : 0);

    fprintf(stderr, "source mesh <v e f r> %d %d %d %d\n",
            source_mesh.nglobal_ents(0), source_mesh.nglobal_ents(1),
  ...
  SupportResults support =
      searchNeighbors(source_mesh, target_mesh, cutoffDistance);
  ...
  Write<LO> supports_per_target(target_mesh.nverts(), 0,
                                "number of supports in each target point");

  Kokkos::parallel_for(
      target_mesh.nverts(), KOKKOS_LAMBDA(int i) {
  ...
  target_points.coordinates =
      PointsViewType("Number of local source supports", target_mesh.nverts());
  Kokkos::parallel_for(
      "target points", target_mesh.nverts(), KOKKOS_LAMBDA(int j) {
        target_points.coordinates(j).x = target_coordinates[j * dim];
        target_points.coordinates(j).y = target_coordinates[j * dim + 1];
      });

  Write<Real> exact_target_values(target_mesh.nverts(), 0,
                                  "exact target values");
