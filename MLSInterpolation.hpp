...
Write<Real> mls_interpolation(const Reals source_coordinates, const Reals target_coordinates, const SupportResults& support, const LO& dim, const Real& cutoff_radius){
   ...
   // Kokkos::RangePolicy<Kokkos::DefaultExecutionSpace> range_policy(1, nvertices_target);
  
    Kokkos::View<size_t*> shmem_each_team("stores the size required for each team", nvertices_target);
    //Write<int> shmem_each_team(nvertices_target, 0, "stores the size required for each team member");
    
    Kokkos::parallel_for("calculate the size required for scratch for each team", nvertices_target, KOKKOS_LAMBDA (const int i ){
	...
	shmem_each_team(i) = total_shared_size;
     ...
     size_t shared_size;
     Kokkos::parallel_reduce("find_max", nvertices_target, KOKKOS_LAMBDA(const int i, size_t& max_val_temp) {
    ...
    Kokkos::parallel_for("MLS coefficients", tp.set_scratch_size(0, Kokkos::PerTeam(shared_size)), KOKKOS_LAMBDA(const member_type& team){
